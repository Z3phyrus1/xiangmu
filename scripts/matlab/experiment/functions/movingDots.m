function movingDots(scr,dots,duration, visual, decision, marker, dot_code, dot_end)
...
%     end
    ...
    % 打marker：首帧打刺激开始，最后一帧打刺激结束
    if frameNum == 1
        % 第一帧：刺激开始
        send_marker(marker, dot_code);
        WaitSecs(0.005);
        clear_marker(marker);
    elseif frameNum == nFrames
        % 最后一帧：刺激结束
        send_marker(marker, dot_end);
        WaitSecs(0.005);
        clear_marker(marker);
    end
end
