function [design] = runTrials(design, datFile, resDir, scr, visual, marker)
...
while do_practice
    % 创建练习专用的marker设置（不打marker）
    marker_practice = marker;  % 复制原始marker结构
    marker_practice.enabled = false;  % 关闭练习阶段的marker
    ...
    for i = 1:npt
        ...
        % 修改这一行，传入 marker_practice
        runSingleTrial(ptd, scr, visual, design, marker_practice);
...
for b = 1:design.nBlocks
    ...
    while t < ntt
        ...
        % run single trial，传入marker
        [data1, data2] = runSingleTrial(td, scr, visual, design, marker);
        ...
        % ############################
        A = linspace(0.5,1,5);
        design.iti = A(randi(numel(A),1,1));
        % #############################
