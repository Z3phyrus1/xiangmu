...
# 创建个体被试图表保存目录
if(!dir.exists("D:/xiangmu/figures/behavior_for_behavior/individual_wrong")){
  dir.create("D:/xiangmu/figures/behavior_for_behavior/individual_wrong", recursive = TRUE)
}
...
plot_individual_subject <- function(subject_id) {
  ...
  # 保存为PDF文件
  pdf_file <- paste("D:/xiangmu/figures/behavior_for_behavior/individual_wrong/subject_", subject_id, ".pdf", sep = "")
