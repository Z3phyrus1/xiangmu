...
# ------------------------------------------------------#
# load data
d <- read.table("D:/xiangmu/data/behavior_for_behavior/data_wide_wmPred.txt",header=T, sep=";")
...
# make big plot
library(ggpubr)
pdf("D:/xiangmu/figures/behavior_for_behavior/pred_diff_individual.pdf",width=6.5,height=5)
ggarrange(pl1, pl2,
          ncol = 2, nrow = 1,
          labels=c("A","B"))
dev.off()
